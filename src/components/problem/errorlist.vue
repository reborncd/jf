          ...
          <el-form :model="form" label-width="120px">
            ...
            <el-row :gutter="24">
              <el-form-item label="故障描述" style="position: relative;">
                ...
                <p v-for="(item,index) in popup.fileList">{{item.name}}
                  <i @click="popup.fileList.splice(index,1)" class="el-icon-close"></i>
                </p>
                </el-upload>
              </el-form-item>
            </el-row>
...
//         搜索
      ...
      getFile(e){
        ...
        this.$set(this.popup, "fileList", file)
        console.log(this.popup.fileList)
      },
//          提交故障单
      subForm(){
        //提交故障提交单
        if (!this.popup.priperty2) {
          this.$warn('请选择故障等级');
          return;
        }
...
//      		let params = new URLSearchParams();
        ...
        params.append("files", this.popup.fileList);
        ...
        this.$axios.post("/fault/submit", params, config).then((res) => {
          let data = res.data;
          if (data.code == 200) {
            this.$success("操作成功！");
...
//               this.$axios.post("/fault/upload", params).then((res) => {
//                  let data = res.data;
//                  if (data.code == 200) {
//                      this.$success("操作成功！");
//                      this.errorVisible=false;
